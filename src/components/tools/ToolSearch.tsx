...
export default function ToolSearch({ className = "", placeholder, locale: localeProp }: { className?: string; placeholder?: string; locale?: Locale }) {
  ...
  useEffect(() => {
    function handlePointerDown(event: PointerEvent) {
      ...
      if (search && !search.contains(target)) { setIsFocused(false); setActiveIndex(-1); }
    }
  ...
  function handleKeyDown(event: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (event.key === "Enter") { event.preventDefault(); openResult(activeIndex >= 0 ? activeIndex : 0); }
  }
