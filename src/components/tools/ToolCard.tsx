...
export default function ToolCard({ tool, categoryName }: ToolCardProps) {
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-5 text-lg font-semibold text-[var(--foreground)]">{getToolName(tool, locale)}</h3>
      <p className="mt-2 text-sm leading-6 text-[var(--muted)]">{getToolDescription(tool, locale)}</p>
